...
function resolveBasePriceFromData(sejourData, startDate) {
  if (!sejourData) return 0;

  if (Array.isArray(sejourData.dates) && startDate) {
    const selectedDateEntry = sejourData.dates.find((entry) => {
      if (typeof entry !== "object" || !entry.startDate) return false;
      return entry.startDate === startDate;
    });

    if (selectedDateEntry && typeof selectedDateEntry === "object") {
      if (selectedDateEntry.basePrice !== undefined) return selectedDateEntry.basePrice;
      if (selectedDateEntry.price !== undefined) return selectedDateEntry.price;
      if (selectedDateEntry.tarif !== undefined) return selectedDateEntry.tarif;
    }
  }

  return sejourData.basePrice ?? 0;
}
...
export default function SejourDetail() {
      ...
      try {
        ...
        if (Array.isArray(data.dates) && data.dates.length > 0) {
          ...
          if (typeof firstDateOption === "object") {
            ...
            setBasePrice(resolveBasePriceFromData(data, firstDateOption.startDate));
          } else {
            setSelectedStartDate(firstDateOption);
            setSelectedEndDate("");
            setBasePrice(resolveBasePriceFromData(data, firstDateOption));
          }
        } else {
  ...
  useEffect(() => {
    ...
    if (!isRoundTrip) {
      const station = sejour.stations.find((s) => s.name === selectedDepartureCity);
    ...
    const dep = sejour.stations.find((s) => s.name === selectedDepartureCity);
    const ret = sejour.stations.find((s) => s.name === selectedReturnCity);
  ...
  const handleDateChange = (event) => {
    ...
    setBasePrice(resolveBasePriceFromData(sejour, chosenStartDate));
  };
  ...
  return (
    <section className="relative bg-white">
      <GenericSejour sejourData={sejour} reservationPrice={basePrice} feedback={feedback} />
      <SejourTabs
        sejour={sejour}
        feedback={feedback}
