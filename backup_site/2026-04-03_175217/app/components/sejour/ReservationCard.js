...
function parsePriceValue(rawPrice) {
  ...
  if (typeof rawPrice === "string") {
    ...
    const normalized = numericValues.map((value) => Number(value.replace(",", "."))).filter((value) => !Number.isNaN(value));
...
export default function ReservationCard({
  ...
  const computeTransportPrice = () => {
    if (!sejour?.stations?.length) return 0;

    const stationByName = (name) => sejour.stations.find((s) => s.name === name);
  ...
  const parsedBasePrice = parsePriceValue(reservationPrice);
  ...
  const baseDisplay =
    parsedBasePrice.min === parsedBasePrice.max
      ? `${formatPrice(parsedBasePrice.min)} €`
      : `${formatPrice(parsedBasePrice.min)} - ${formatPrice(parsedBasePrice.max)} €`;
                ...
                {sejour?.stations?.map((station) => {
                  ...
                  return (
                    <option key={`dep-${station.name}`} value={station.name}>
                      {station.name} (+{formatPrice(shownPrice)} €)
                    </option>
                  );
                })}
              </select>
