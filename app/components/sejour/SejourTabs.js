...
export default function SejourTabs({
  ...
  const galleryImages = useMemo(() => {
    ...
    const hasExplicitGalleryField =
      Boolean(sejour) && Object.prototype.hasOwnProperty.call(sejour, "galleryImages");
    const explicitGallery = Array.isArray(sejour?.galleryImages) ? sejour.galleryImages : [];
    explicitGallery.forEach((entry) => {
      if (typeof entry === "string") pushImage(entry);
      else if (entry && typeof entry === "object") {
        pushImage(entry.src || entry.url || entry.image || entry.photoUrl);
      }
    });
    if (hasExplicitGalleryField) return collected;
