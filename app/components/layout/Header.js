...
const injectCss = `
  ...
  @media (max-width: 1180px) { .hdr-top-link { display: none; } }
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      ...
      try {
        ...
        if (mounted) setIsAdminSession(adminSnap.exists());
      } catch {
        if (mounted) setIsAdminSession(false);
      }
    });
      ...
      <header style={{ position: "fixed", inset: "0 0 auto 0", zIndex: 50, borderBottom: "1px solid #e9ddd0", background: "rgba(255,255,255,0.97)", backdropFilter: "blur(12px)" }}>
        ...
        <div style={{ background: "#B8336A" }}>
          <div className="relative mx-auto flex max-w-[1240px] items-center justify-center px-4 py-1 sm:h-8 sm:py-0">
            ...
            {isAdminSession ? (
              <Link
                href="/dashboard"
                className="absolute right-2 top-1/2 hidden -translate-y-1/2 items-center rounded-full border border-white/35 bg-white/15 px-2.5 py-0.5 text-[0.62rem] font-bold uppercase tracking-[0.08em] text-white transition hover:bg-white/25 sm:inline-flex"
              >
                Dashboard
              </Link>
            ) : null}
        ...
        <div className="hdr-main-bar">
          ...
          <nav className="hidden md:flex flex-1 items-center hdr-nav-inner" style={{ gap: 0, flexWrap: "nowrap" }}>
            ...
            <Link href="/blog" className="hdr-blog-link">
              BLOG
            </Link>
          </nav>
          ...
          <div className="hidden md:flex hdr-ctas">
            ...
            {/* Réserver — pill rose */}
            <Link href="/reserver" className="hdr-resa-pill">
