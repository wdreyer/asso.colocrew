...
const DEFAULT_ACCOUNTING = {
  ...
  landing2026: {
    bankMonthly: [
      ...
      { month: "Juillet", inflows: 77882.21, outflows: 72601.33, note: "Pic séjours été" },
      { month: "Août", inflows: 58148.55, outflows: 57999.93, note: "Pic séjours été" },
      { month: "Septembre", inflows: 23595.16, outflows: 24447.42, note: "Export Qonto au 10/09" },
      { month: "Rapprochement", inflows: 982.35, outflows: 0, note: "Rapprochement avec le solde bancaire final visé" },
    ],
    bankCategories: [
    ...
    expenseCategories: [
      { label: "Achats, alimentation, fournitures et opérations", account: "60", amount: 24638.35 },
      { label: "Hébergements, locations, activités et prestataires", account: "61", amount: 141182.09 },
      { label: "Transports, communication, administratif, banque et technologies", account: "62", amount: 65905.38 },
      { label: "Impôts et taxes", account: "63", amount: 2258 },
  ...
  forecast: {
    ...
    note: "Budget 2026 repris depuis l'export Qonto du 10/09/2026, les aides VACAF réelles transmises et le solde bancaire final visé à 11 276 €. Les postes restent rangés dans les catégories du bilan comptable, avec les avances, prêts et virements internes isolés hors résultat dans l'atterrissage.",
  },
  balance: {
    assets: [
      { label: "Immobilisations", account: "2", amount: 2700 },
...
function mergeAccountingDraft(saved) {
  ...
  return accountingWithAutomaticReports({
    ...
    production: {
      selectedPlanId: saved.production?.selectedPlanId || DEFAULT_ACCOUNTING.production.selectedPlanId,
      plans: Array.isArray(saved.production?.plans) && saved.production.plans.length
        ? saved.production.plans
        : DEFAULT_ACCOUNTING.production.plans,
    },
...
function ProductionBudgetModule({
  accounting,
  setAccounting,
  onlineStays,
  rows,
  ...
  const production = accounting.production || DEFAULT_ACCOUNTING.production;
  const plans = (Array.isArray(production.plans) && production.plans.length ? production.plans : DEFAULT_ACCOUNTING.production.plans)
    .map(normalizeProductionPlan);
  ...
  const writePlans = (nextPlans, nextSelectedId = selectedPlanId) => {
    setAccounting((previous) => ({
      ...previous,
      production: {
        ...(previous.production || DEFAULT_ACCOUNTING.production),
  ...
  const addPlan = () => {
    ...
    const plan = normalizeProductionPlan({
      ...
      id,
      name: `Nouvelle simulation ${plans.length + 1}`,
      linkedStayId: "",
      mode: "simulation",
    });
    writePlans([...plans, plan], id);
  };
  ...
  const duplicatePlan = () => {
    ...
    const plan = normalizeProductionPlan({
      ...selectedPlan,
      id,
      name: `${selectedPlan.name || "Simulation"} - copie`,
    });
    writePlans([...plans, plan], id);
  };
  ...
  const addExpense = () => {
    updatePlan({
      expenses: [
        ...
        { label: "Nouvelle dépense", category: "Autre", unit: "fixed", quantity: 1, unitAmount: 0 },
      ],
    });
  };
  ...
  return (
    <section className="production-panel">
      <div className="finance-section-head">
        <div>
          <h2>Production et rentabilité des séjours</h2>
          <p>Simule un séjour ou relie le budget à une fiche séjour en vente. Les recettes, dépenses et coûts RH se recalculent automatiquement.</p>
        </div>
        <div className="accounting-actions">
          <button type="button" className="dash-btn dash-btn-secondary" onClick={addPlan}>Nouvelle simulation</button>
          <button type="button" className="dash-btn" onClick={onSave}>Enregistrer</button>
        </div>
      </div>
          ...
          {plans.map((plan) => {
            ...
            return (
              <button
                ...
                onClick={() => setAccounting((previous) => ({
                  ...previous,
                  production: { ...(previous.production || DEFAULT_ACCOUNTING.production), selectedPlanId: plan.id, plans },
                }))}
              >
                <strong>{plan.name}</strong>
                <span>{plan.stayCode || "Simulation"} · {plan.week || "Toutes"} · {currency(planComputed.margin)}</span>
              </button>
            );
          })}
        </aside>
        ...
        <div className="production-main">
          <section className="production-card">
            <div className="production-card-head">
              <div>
                ...
                <p>Prix de vente, nombre d'enfants, durée et lien avec les séjours en ligne.</p>
              </div>
              <div className="accounting-actions">
            ...
            <div className="production-form-grid">
              <label><span>Nom du budget</span><input value={selectedPlan.name || ""} onChange={(event) => updatePlan({ name: event.target.value })} /></label>
              <label>
                <span>Séjour en ligne</span>
                <select value={selectedPlan.linkedStayId || ""} onChange={(event) => applyOnlineStay(event.target.value)}>
                  <option value="">Simulation indépendante</option>
                  {onlineStays.map((stay) => (
                    <option key={stay.id} value={stay.id}>{stay.name || stay.id}</option>
                  ))}
                </select>
              </label>
          ...
          <section className="production-card">
            <div className="production-card-head">
              <div>
                <h3>RH</h3>
                <p>Utilise les contrats existants si disponibles, sinon la simulation DS/anims ci-dessous.</p>
              </div>
          ...
          <section className="production-card">
            <div className="production-card-head">
              <div>
                <h3>Dépenses de production</h3>
                <p>Chaque poste peut être un forfait, une quantité libre, un coût par enfant, par personne ou par durée.</p>
              </div>
              <button type="button" className="dash-btn dash-btn-secondary" onClick={addExpense}>Ajouter une dépense</button>
            </div>
...
export default function Finances() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const [snapshot, summarySnapshot, transportsSnapshot, contractsSnapshot, onlineStaysSnapshot, accountingSnapshot] = await Promise.all([
